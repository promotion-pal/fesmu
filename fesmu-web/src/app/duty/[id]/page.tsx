...
export default function RecordDutyPage() {
  ...
  const fetchDutyData = useCallback(async () => {
    ...
    try {
      ...
      // Не показываем toast при ошибках автоматического обновления
    ...
    <CommonWrapper back={ROUTE.DUTY()} styleWrapper="container pb-10">
      ...
      <CommonFrom
        ...
        onSubmit={async (data) => {
          try {
            ...
            // Обработка специфических ошибок с сервера
            if (error.response?.data?.message) {
              const serverMessage = error.response.data.message;
              setFormError(serverMessage);
              toast.error(serverMessage);
            } else {
              setFormError("Не удалось записаться. Попробуйте еще раз.");
      ...
      >
        {(form) => {
          ...
          // Восстанавливаем данные формы при изменении id (если нужно)
          useEffect(() => {
          ...
          return (
            <>
              ...
              >
                {isSubmittingRef.current ? "Запись..." : "Записаться"}
              </Button>
            </>
          );
        }}
